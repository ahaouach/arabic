...
/**
 * Build N rounds where targets are sampled without replacement first
 * (so every colour is heard at least once before repeats). For each
 * round, N-1 distractors are drawn from the remaining palette and the
 * options list is shuffled independently so the correct position varies.
 */
function buildRounds(
  ...
  while (targets.length < totalRounds) {
    if (remaining.length === 0) remaining = shuffle(colors, seed + targets.length);
  ...
  return targets.map((target, i) => {
    ...
    const distractors = pickRandom(distractorPool, optionsPerRound - 1, seed + i + 1);
...
export default function ListenPickZone({
  ...
  const rounds = useMemo<Round[]>(() => {
    if (seed === null) return [];
    return buildRounds(colors, zone.rounds, zone.optionsPerRound, seed);
  }, [colors, zone.rounds, zone.optionsPerRound, seed]);
  ...
  const check = useCallback(() => {
    ...
    if (selectedKey === round.target.key) {
      ...
      // Let the child retry the same round.
      window.setTimeout(() => {
        setSelectedKey(null);
        setRevealed(false);
        setFeedback(null);
  ...
  return (
    <section
      ...
      aria-labelledby="colors-listen-title"
    >
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <h3
            id="colors-listen-title"
            className="text-2xl font-black text-gray-900"
            lang="ar"
            dir="rtl"
            style={{ fontFamily: '"Amiri", "Noto Naskh Arabic", serif' }}
          >
            {zone.title ?? "اِسْتَمِعْ وَاخْتَرِ اللَّوْنَ"}
          </h3>
          {zone.description && (
            <p
          ...
          >
            <div className="flex items-center justify-center">
              <motion.button
                ...
                aria-label="Play the colour name"
              >
