...
interface FeedbackOverlayProps {
  ...
  kind: FeedbackKind;
  /** Arabic vowelised message. Fallbacks are provided per-kind. */
  message?: string;
  /** Controls how long the overlay stays visible after mount. */
  ttlMs?: number;
  /** Fires when the TTL expires (used to clear the parent's `kind` prop). */
  onDismiss?: () => void;
}
...
/**
 * Transient top-of-zone badge for correct / wrong / hint feedback.
 *
 * Parent controls visibility via the `kind` prop. The overlay auto-
 * dismisses after `ttlMs` and signals the parent to reset its state via
 * `onDismiss`. Relies on AnimatePresence for an exit animation — safe
 * with `prefers-reduced-motion` (collapses to an opacity fade).
 */
