...
interface ScoreTrackerProps {
  /** Number of completed rounds / correct actions. */
  completed: number;
  /** Total rounds expected. */
  total: number;
  /** Optional Arabic label. Defaults to "التَّقَدُّمُ". */
  labelAr?: string;
  /** Tinted accent colour when the tracker is mostly green (on pace). */
...
/**
 * Compact progress pill — "3 / 5 ⭐" with a gradient bar underneath.
 *
 * Intended for zone-local progress (one zone's rounds). The lesson-
 * level `StageProgress` component already covers the 6-zone outer
 * journey; this one is for *within* a zone.
 */
