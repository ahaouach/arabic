...
import type {
  FamilyColorLettersZone as FamilyColorLettersZoneType,
  FamilyWord,
  WordLetter,
...
export interface ColorLettersZoneProps {
  zone: FamilyColorLettersZoneType;
  words: FamilyWord[];
  colors: ArabicColor[];
...
interface Target {
  base: string;
  name: string; // letter's Arabic name for audio
  color: ArabicColor;
}
...
/**
 * Zone 6 — Color the letters of the word.
 *
 * Reuses the pre-split `word.letters` from the seed (base + harakat +
 * pre-combined display) so no runtime Unicode splitting happens.
 *
 * Each round: 1-2 **target bases** are picked from the distinct base
 * letters of the word, paired with distinct colours. Child works
 * through targets sequentially — all occurrences of base 1 get
 * colour 1, then base 2 gets colour 2, then the round completes
 * with a full-word pronunciation.
 */
...
function generateRounds(
  ...
  // If we don't have enough distinct words, cycle with extra seed variation.
  while (wordsByRound.length < zone.rounds) {
    const more = pickRandom(words, Math.min(zone.rounds - wordsByRound.length, words.length), Math.floor(rand() * 0xffffffff));
    wordsByRound.push(...more);
  }

  return wordsByRound.map((word, i) => {
    const bases = distinctBases(word.letters);
    // Clamp: can't ask for more distinct targets than the word has.
...
export default function ColorLettersZone({
  ...
  /** Map letter-index-in-word → hex fill. */
  ...
  // Auto-play instruction audio when the target changes.
  ...
  const handleLetterTap = (letter: WordLetter, index: number) => {
    ...
    if (filledIndices[index]) return; // already coloured
    ...
    if (letter.base === currentTarget.base) {
      ...
      // Any more occurrences of this target still uncoloured?
      const remainingForTarget = round.word.letters.some(
        (l, i) => l.base === currentTarget.base && !nextFilled[i],
      );
      if (remainingForTarget) return;

      // All occurrences of this target coloured — move to next target or round.
      if (targetIndex + 1 < round.targets.length) {
        setTargetIndex((i) => i + 1);
        return;
      }

      // Round cleared — celebrate, pronounce the word, advance.
      setFeedback("correct");
  ...
  return (
    <motion.section
      aria-labelledby="family-color-letters-title"
      ...
      {finished ? (
        <div className="flex flex-col items-center gap-4">
          ...
          >
            ...
            <span>إِنْهَاءُ الْعَائِلَةِ</span>
          </motion.button>
        </div>
      ) : (
        round && (
