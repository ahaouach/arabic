...
import type {
  FamilyListenPickZone as FamilyListenPickZoneType,
  FamilyMember,
 ...
 *
 * Flow per round:
 *   1. Pick the round's target (without replacement across the
 *      session's rounds) + N-1 distractors. Same-category distractors
 *      preferred when the config enables it and the pool is big enough.
 *   2. Auto-play target audio on round entry.
 *   3. Child taps an option.
 *      ✓  → green ring + correct feedback + auto-advance after 1.2 s.
 *      ✗  → rose ring + shake + replay target audio; stays in round.
 *   4. After the final round: report (correct / total) to the lesson.
 */
function pickDistractors(
  ...
  if (preferSameCategory && target.category) {
    ...
    // Not enough same-category — fill from same-category first, then the rest.
...
function generateRounds(
  ...
  // Targets without replacement — if rounds > members.length we cycle.
  const rand = mulberry32(seed);
  const targets: FamilyMember[] = [];
  let pool: FamilyMember[] = [];
  for (let i = 0; i < roundCount; i++) {
    if (pool.length === 0) {
      pool = shuffle(members, Math.floor(rand() * 0xffffffff));
    }
    targets.push(pool.shift() as FamilyMember);
  }
...
export default function ListenPickZone({
  ...
  // Auto-play the target audio when we enter a new round.
  ...
  const title = zone.title ?? "اِسْتَمِعْ وَاخْتَرِ الْفَرْدَ";
  const instruction = "اِضْغَطْ عَلَى الصُّورَةِ الصَّحِيحَةِ";

  return (
    <motion.section
      aria-labelledby="family-listen-pick-title"
