 ...
 * Zone 1 — Discovery.
 *
 * Every vocabulary item rendered as a tap-to-play card. Order is
 * shuffled on mount; the "Shuffle" button regenerates the seed for
 * replay. Visiting at least one card marks the zone complete so kids
 * never get stuck if they only want to skim.
 */
...
export default function DiscoveryZone({
  ...
  const ordered = useMemo<VocabularyItem[]>(() => {
  ...
  }, [visited, announced, onComplete]);

  const playItem = (it: VocabularyItem) => {
  ...
  return (
    <motion.section
      aria-labelledby="vocab-discovery-title"
    ...
    >
      ...
      <div className="mb-6 flex flex-wrap items-center justify-center gap-3">
        ...
        >
          ...
          <span dir="rtl" lang="ar" style={{ fontFamily: '"Amiri", "Noto Naskh Arabic", serif' }}>
            خَلْطٌ
          </span>
        </button>
        <button
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4">
        {ordered.map((it, i) => (
          <VocabularyCard
            ...
            onClick={() => playItem(it)}
            index={i}
            status={visited.has(it.key) ? "correct" : "idle"}
          />
        ))}
      </div>
